...
const COCKROACHDB_DATABASE: &'static str = "test";
...
impl CockroachInstance {
    ...
    pub async fn cleanup(&mut self) -> Result<(), anyhow::Error> {
        ...
        if let Some(child_process) = self.child_process.as_mut() {
            ...
            let success = 0 == unsafe { libc::kill(pid as libc::pid_t, libc::SIGTERM) };
            if !success {
    ...
    fn drop(&mut self) {
        ...
        if self.child_process.is_some() || self.temp_dir.is_some() {
            ...
            if let Some(temp_dir) = self.temp_dir.take() {
                ...
                eprintln!("WARN: temporary directory leaked: {}", path.display());
            }
        }
    }
}
